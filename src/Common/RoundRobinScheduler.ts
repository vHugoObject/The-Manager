...
export const totalRoundRobinMatches = flowAsync(over([totalRoundRobinRounds, matchesPerRoundOfRoundRobin]), spread(multiply))
export const totalDoubleRoundRobinMatches = flowAsync(totalRoundRobinMatches, multiplyByTwo)

export const modularArithmetic = curry((arithmeticFunction: (arg: number) => number, rangeMax: number, num: number): number => {
...
export const firstWeekOfRoundRobinWithEvenNumberClubs = (clubs: number): [number, Array<[number, number]>] => {
  const adjustedRangeMax = multiplyByTwo(clubs) - 1
  return [clubs, flowAsync(
    half,
    range(1),
...
export const everyWeekAfterFirstWeekofRoundRobin = ([clubs, firstRound]: [number, Array<[number, number]>]): Array<Array<[number, number]>> => {
  ...
  return flowAsync(
    range(1),
    reduce((rounds: Array<Array<number>>, _: number) => {
      const [firstMatch, allOtherMatches] = flowAsync(last, over([head, tail]))(rounds)
      return concat(rounds, [concat([flowAsync(map((num: number) => num !== 0 ? num + 1 : num), reverse)(firstMatch)],
...
export const roundRobinScheduler = flowAsync(firstWeekOfRoundRobinWithEvenNumberClubs, everyWeekAfterFirstWeekofRoundRobin)


export const doubleRoundRobinScheduler = flowAsync(roundRobinScheduler,
  (firstHalf: Array<Array<[number, number]>>): Array<Array<[number, number]>>  => flowAsync(
    flowAsync(lastTwoArrayValues,
      map((round: Array<[number, number]>): Array<[number, number]> => flowAsync(firstTwoArrayValues,
	map(reverse),
	concat(takeRight(round.length - 2, round)))(round)
      )),
