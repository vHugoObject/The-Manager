...
import { getCountryDomesticLeagues, pickCountries } from "../../Countries/CountryUtilities"
import { getCompetitionClubs, pickDomesticLeagues, filterDomesticLeaguesByID } from "../../Competitions/CompetitionUtilities"
import { getClubSquad, pickClubs, filterClubsByID } from "../../Clubs/ClubUtilities"
...
describe("CreateEntities", async () => {
  ...
  test.prop([
    ...
    const expectedPositionCounts = getTotalPlayersToGenerateBasedOnGivenComposition(BASECLUBCOMPOSITION, 0, expectedClubsCount);
    

    const expectedLastPositionIDs = getExpectedLastIDsForMultiplePositions(expectedPositionCounts)
    ...
    const [actualIDsSet, expectedIDsSet]: Array<Set<string>> = convertArrayOfArraysToArrayOfSets([flatten(actualPlayerIDs), expectedLastPositionIDs])
  ...
  test.prop([
    ...
    const actualEntities: Record<string, Entity> = await createEntities(testBaseEntities)



    const getActualCountriesCount = flowAsync(pickCountries, size)
    const getActualDomesticLeaguesCountFromCountries = flowAsync(pickCountries, flatMap(getCountryDomesticLeagues), filterDomesticLeaguesByID ,size)
    const getActualClubsCountFromDomesticLeagues = flowAsync(pickDomesticLeagues, flatMap(getCompetitionClubs), filterClubsByID, size)
    const getActualPlayersCountFromClubs = flowAsync(pickClubs, flatMap(getClubSquad), filterPlayersByID, size)

    const [actualCountriesCount, actualDomesticLeaguesCountFromCountries,
      actualClubsCountFromDomesticLeagues,
      actualPlayersCountFromClubs
    ] = flowAsync(      
      over([getActualCountriesCount,
    ...
    const {countries: expectedCountriesCount, domesticLeagues: expectedDomesticLeaguesCount, clubs: expectedClubsCount} = getTestBaseEntitiesCount(testBaseEntities)
    ...
    expect(actualDomesticLeaguesCountFromCountries).toEqual(expectedDomesticLeaguesCount)
