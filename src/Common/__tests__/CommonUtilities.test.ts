...
import { normalizePercentages, weightedMean, weightedRandom } from '../CommonUtilities' 

describe("CommonUtilities test suite", async() => {

  const arrayOfRandomIntegers = async(arrayLength: number): Promise<Array<number>> => {
      return await Promise.all(range(0,arrayLength).map(async(_) => {
	return simpleFaker.number.int({min: 1, max: 100})
      }))}

  const arrayOfRandomFloats = async(minimum: number, arrayLength: number): Promise<Array<number>> => {
  ...
  test("test normalizePercentages", async() => {
    ...
    await Promise.all(testArrays.map(async(testArray: Array<number>) => {
      const actualPercentages: Array<number> = await normalizePercentages(testArray)
  ...
  test("test weightedMean", async() => {
    ...
    await Promise.all(zip(testArraysOfPercentages,
      testArraysOfIntegers)
      .map(async([testPercentages,testIntegers]) => {
	const actualMean: number = await weightedMean(testPercentages, testIntegers)
  ...
  test("test weightedRandom", async() => {
    ...
    await Promise.all(zip(testArraysOfPercentages,
      testArraysOfIntegers)
      .map(async([testPercentages,testIntegers]) => {
	const actualChosenNumber: number = await weightedRandom([testPercentages, testIntegers])
	const setOfTestIntegers: Set<number> = new Set(testIntegers)
	expect(setOfTestIntegers.has(actualChosenNumber)).toBeTruthy();
	expect(testPercentages[testIntegers.indexOf(actualChosenNumber)]).toBeGreaterThan(0)
      }));
