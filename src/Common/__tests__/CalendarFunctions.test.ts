...
describe("Calendar", async () => {
  ...
  test.prop([
    ...
    async (testSeason,) => {
      ...
      const testStartsAndEnds: Array<Date> = over([getThirdSundayOfAugust,
	getLastDayOfAugust,
	...
	getJuneFifteenOfNextYear,
	getLastDayOfJuneOfNextYear])(testSeason) as Array<Date>
	...
	[false, false, false, false, true],
      ...
      const testFunctionTuples: Array<[(season: number) => Date, (season: number) => Date]> = [
	...
	[getJuneFifteenOfNextYear, getLastDayOfJuneOfNextYear]
      ...
      const actualTransferWindowFunctions = createSeasonWindows(testFunctionTuples, testSeason);

      const asserter = (testFunction: (date: Date) => boolean, testDate: Date, expectedValue: boolean) => {
	if (!testFunction(testDate)) {
	}
	expect(testFunction(testDate)).toBe(expectedValue)
      }
      flowAsync(
	map((expectedBooleanValues: Array<boolean>) => zip(testDates, expectedBooleanValues)),
	zip(actualTransferWindowFunctions),
	map(([testFunction, testDateAndExpectedValues]: [(date: Date) => boolean, Array<[Date, boolean]>]) => {
  ...
  test.prop([
    fc.integer({min: 2000, max: 2100})
      .chain((testSeason: number) => {
	return fc.tuple(fc.date({
    ...
    async (testDateTestSeasonAndExpectedSeasonEndDate) => {
      const [testDate, testSeason, expectedSeasonEndDate]: [Date, number, Date] = testDateTestSeasonAndExpectedSeasonEndDate
      const testFunctionTuples: Array<[(season: number) => Date, (season: number) => Date]> = [
	[getThirdSundayOfAugust, getLastDayOfAugust],
	[getStartOfNextYear, getFirstMondayOfFebruaryOfNextYear],
	[getJuneFifteenOfNextYear, getLastDayOfJuneOfNextYear]
      ]
      const testDefaultIsTransferWindowOpen = isTransferWindowOpen(testFunctionTuples)
      const actualResult: boolean = testDefaultIsTransferWindowOpen([testSeason, testDate])
      expect(actualResult).toBeTypeOf('boolean')
      expect(defaultIsTransferWindowOpen([testSeason, testDate])).toBeTypeOf('boolean')
  ...
  test.prop([
    ...
    async (testDateTestSeasonAndExpectedSeasonEndDate) => {
      const [testDate, testSeason, expectedSeasonEndDate]: [Date, number, Date] = testDateTestSeasonAndExpectedSeasonEndDate
      const actualDaysLeft: number = getDaysLeftInCurrentSeason(testSeason, testDate)
      const testDatePlusActualDaysLeft: Date = addDays(actualDaysLeft)(testDate)
      flowAsync(
	map(format("MM/dd/yyyy")),
	([actual, expected]: [Date, Date]) => expect(isDateEqual(actual, expected)).toBeTruthy()
  ...
  test.prop([
    ...
    async (testDateAndExpectedSeasonEndDate) => {
      const [testDate, expectedSeasonEndDate]: [Date, Date] = testDateAndExpectedSeasonEndDate
