...
import { fastCheckTestSeasonAndClubNumber } from "../TestDataGenerators";
import { getCountOfIDParts, getCountOfObjectKeys } from "../Getters";
import { createClubID, splitUnderscoresMapAndSum } from "../Transformers";

describe("ClubIDTransformers test suite", () => {
  test("createClubID", () => {
    fc.assert(
      fc.property(fc.gen(), (fcGen) => {
        const [testSeason, testClubNumber] =
        ...
        const expectedCountOfIDParts: number =
          getCountOfObjectKeys(CLUBIDINDICES);
