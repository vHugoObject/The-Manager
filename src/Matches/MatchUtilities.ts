...
// need to check if the team has a goalkeeper
// then add a test for that
...
console.log
  ...
  const goalkeeper: Player = players.filter((player) => player.PositionGroup == PositionGroup.Goalkeeper)[0]

  const goalkeepingRating: number = playerRating(goalkeeper, goalkeepingCategories);
  
  const outfieldPlayerDefenseRatings: Array<number> = players.filter((player) => player.PositionGroup != PositionGroup.Goalkeeper)
    .map((player: Player) => {
      return playerRating(player, defenseCategories)
    })

  return sum(outfieldPlayerDefenseRatings.concat(goalkeepingRating))/11;
...
export const calculateAttackStrength = (players: Array<Player>): number => {
  const attackCategories = new Set(["ballSkills", "mentalSkills",
    ...
    const averageByCategory = (player: Player, categories: Set<string>): Array<number> => {
      return Object.entries(player.Skills).filter(([key,_]) => categories.has(key))
	.map(([_, value]) => {
    ...
    return sum(allAttackRatings)/11
}

export const calculateHomeStrength = (homeAtt: number, awayDef: number): number => {
  const U = -1.035
...
export const calculateAwayStrength = (awayAtt: number, homeDef: number): number => {
  const U = -1.035
...
export const weibullCDFGoals = (shape: number, teamStrength: number, goals: number): number => {
...
export const calculateJointProbability = (homeProb: number, awayProb: number, theta: number) => {
  const home = homeProb
  const away = awayProb
  return Math.log(1 + ((Math.exp(-theta * home) - 1) * (Math.exp(-theta * away) - 1)) / (Math.exp(-theta) - 1)) * -(1 / theta);
}


export const createJointProbabilitiesMatrixForGoals = (homeTeamStrength: number,
  ...
  return scoreMatrix.map(([homeScore, awayScore]) => {
    const homeProb: number = weibullCDFGoals(shape, homeTeamStrength, homeScore)
    const awayProb: number = weibullCDFGoals(shape, awayTeamStrength, awayScore)

    const [normalizedHomeProb, normalizedAwayProb] = normalizePercentages([homeProb, awayProb])
    const totalProb: number = homeProb + awayProb;
    const jointProbability: number = calculateJointProbability(normalizedHomeProb, normalizedAwayProb, theta);
    return [[homeScore, awayScore], jointProbability]
  })
}


export const randomScore = (scoreMatrix: Array<[[number,number],number]>): [number, number] => {
  const sortedScoreMatrix = scoreMatrix.sort((a,b) => b[1] - a[1]);  

  const weights: Array<number> = sortedScoreMatrix.map(([_, probability]) => probability);
  const weightedRandom = (items: any[], weights: number[]) => {
    const initialValue: number = 0;
    const cumulativeWeights: number[] = [];
    const _: number = weights.reduce(
    ...
    const maxCumulativeWeight: number = Math.max
(...cumulativeWeights);
    const randomNumber: number = maxCumulativeWeight * Math.random();
    const randomIndex: number = cumulativeWeights.findIndex((weight) => weight >= randomNumber);
...
export const matchExpectedGoals = (scoreMatrix: Array<[[number,number],number]>): [number, number] => {

  const weightedMean = (arrValues: number[], arrWeights: number[]) => {
    // add normalizePercentages,
    const weightSum: number = sum(arrWeights);
    const normalizedWeights: Array<number> = arrWeights.map((weight) => weight/weightSum);
