...
import { zipAll, pipe, isEqual, multiply, over, size, sum } from "lodash/fp";
import { getCountOfStringsFromArray,
  getCountOfIntegersFromArray,
  getCountOfTrueFromArray,
  getCountOfFalseFromArray,
...
import { addOne } from "../../Common/Arithmetic";
import { zipApply,   zipAllAndGetMinOfArrayAtIndex, } from "../../Common/Zippers";
...
describe("TestDataGenerationUtilities test suite", () => {
  const getActualStringIndexAndCountArray = pipe([
    zipAll,
    zipApply([getCountOfStringsFromArray, getCountOfIntegersFromArray, getCountOfIntegersFromArray]),
  ]);
  ...
  test.prop([
    ...
    const actualCharacterCode: number = convertCharacterIntoCharacterCode(actualCharacter);
  ...
  test.prop([fc.integer({ min: 2, max: 1000 }), fc.gen()])(
    ...
    (testArraySize, fcGen) => {
      ...
      const actualIntegerCount: number = getCountOfIntegersFromArray(actualArray);
  ...
  test.prop([fc.integer({ min: 2, max: 1000 }), fc.gen()])(
    ...
    (testArraySize, fcGen) => {
      ...
      const actualStringCount: number = getCountOfStringsFromArray(actualArray);
  ...
  test.prop([
    ...
    (testRangeMin, testRangeSize, fcGen) => {
      const [actualBooleans]: [Array<string>, Array<[string, number]>] =
        fastCheckTestMixedArrayOfBooleansGenerator(fcGen, [
          testRangeMin,
          testRangeSize,
        ]);

      const [actualTrueCount, actualFalseCount, actualSize] = over([
	getCountOfTrueFromArray,
	getCountOfFalseFromArray,
        size,
      ])(actualBooleans);
