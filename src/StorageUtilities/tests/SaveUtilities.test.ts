...
import { convertBaseCountriesToBaseEntities, randomPlayerCompetitonAndClub, fakerToArb, convertToSet,
convertArrayOfArraysToArrayOfSets } from "../../Common/index";
...
describe("SaveUtilities tests", async () => {
  ...
  test.prop(
    ...
    async (
      ...
      const actualValue: Save = await getSaveValue(actualSave);
      expect(actualValue).toStrictEqual(testSave);

      await deleteSave(actualSaveID);
      const db: IDBPDatabase = await openSaveDB();
      const saveKeys: Set<string> = await flowAsync(db.getAllKeys, convertToSet)(saveStore);
  ...
  test.prop(
    ...
    async (
      ...
      const [actualSavesSet, expectedSavesSet] = convertArrayOfArraysToArrayOfSets([actualSaves, [testSave, testSave]])
  ...
  test.prop(
    ...
    async (
      ...
      const [actualSaveIDsSet, expectedSaveIDsSet] = convertArrayOfArraysToArrayOfSets([actualSaveIDs, expectedSaveIDs])
