...
import { OldSavesCards } from "../OldSaves";
      ...
      fc
        .asyncProperty(
          ...
          async (fcGen, testOptionsCount) => {
            const testSaveOptionTuples = fastCheckCreateArrayOfTestSaveOptions(
              fcGen,
              testOptionsCount,
            );
            const TestElement = () => {
            ...
            screen.debug();
            const [expectedRandomSaveName] = pipe([
              compact,
              fastCheckRandomItemFromArray(fcGen),
            ])(testSaveOptionTuples);
